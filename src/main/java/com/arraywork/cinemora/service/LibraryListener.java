...
public class LibraryListener extends DirectoryMonitor.DirectoryListener {
    ...
    //        onChanged(file, count, total, EventSource.SCAN, () -> metadataService.build(file, false));
    //    }

    // 新增文件回调方法
    @Override
    public void onFileCreate(File file) {
        //        onChanged(file, count, total, EventSource.ADD, () -> metadataService.build(file, false));
    }

    // 修改文件回调方法
    @Override
    public void onFileChange(File file) {
        //        onChanged(file, count, total, EventSource.MODIFY, () -> metadataService.build(file, true));
    }

    // 删除文件回调方法
    @Override
    public void onFileDelete(File file) {
        //        onChanged(file, count, total, EventSource.DELETE, () -> metadataService.delete(file));
    }

    // 监听回调方法
    private void onChanged(File file, int count, int total, EventSource event, Consumer consumer) {
        //        TimeUtils.delay(200);
